...
export function createRunShellMutateTool(options: RunShellOptions = {}): ToolDefinition {
  ...
  return {
    ...
    description:
      `Run an approval-gated shell command starting in the current workspace directory. The workspace is the command's initial working directory, not an enforced boundary: an approved command runs with the host user's full authority and can reach paths, networks, and credentials outside the workspace. Use this when inspection is not enough, when the command is arbitrary or unclassified, when you need inline scripts or broader shell expressiveness, or when run_shell_inspect rejects a still-necessary command. Returns structured output with command, exitCode, stdout, stderr, and policy metadata. This tool is governed by host-side execution rules with explicit risk classification and approval instead of a narrow command allowlist. Arbitrary commands are allowed here through approval; do not assume a command is impossible just because inspect refused it.`,
    parameters: buildParameters(),
    execute: (raw: unknown) => runShellCommand(raw, {
