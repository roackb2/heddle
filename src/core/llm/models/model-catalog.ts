...
export class ModelCatalogService {
  static formatOpenAiModelGroups(): string {
  ...
  static formatBuiltInModelGroups(): string {
  ...
  static filterOpenAiModels(query: string): string[] {
  ...
  static isOpenAiAccountSignInModel(model: string): boolean {
    return OPENAI_ACCOUNT_SIGN_IN_MODELS.includes(model.trim());
  }

  static filterBuiltInModels(query: string): string[] {
    const normalized = query.trim().toLowerCase();
    if (!normalized) {
  ...
  static estimateOpenAiContextWindow(model: string): number | undefined {
    return ModelCatalogService.estimateBuiltInContextWindow(model);
  }

  static estimateBuiltInContextWindow(model: string): number | undefined {
    const normalized = model.trim();
    if (!normalized) {
      return undefined;
    }

    return BUILT_IN_CONTEXT_WINDOW_ESTIMATES.get(normalized) ?? ModelCatalogService.inferContextWindowEstimate(normalized);
  }
...
const BUILT_IN_CONTEXT_WINDOW_ESTIMATES = new Map<string, number>(
  COMMON_BUILT_IN_MODELS.map((model) => [model, ModelCatalogService.inferContextWindowEstimate(model)]),
);
