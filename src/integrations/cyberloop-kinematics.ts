...
async function importCyberLoopAdvanced(moduleSpecifier: string | undefined): Promise<CyberLoopAdvancedModule> {
  ...
  try {
    ...
    throw new Error(
      `Unable to load CyberLoop kinematics middleware from ${specifier}. Install cyberloop or set HEDDLE_CYBERLOOP_ADVANCED_MODULE to a local advanced middleware module. ${detail}`,
    );
  }
}
