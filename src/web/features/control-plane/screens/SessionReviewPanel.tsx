...
import { CodeBlock, EmptyState, Pill, SideSection } from '../components/common';
import { DiffViewer } from '../components/DiffViewer';
import { CommandList, TurnListButton } from '../components/lists';
import { className } from '../utils';

type SessionTurn = NonNullable<ChatSessionDetail>['turns'][number];
...
type ReviewFileValue = WorkspaceChangedFileValue | NonNullable<ChatTurnReview>['files'][number];
type ExpandedDiffFile = {
  title: string;
  diff?: WorkspaceFileDiff['diff'];
  patch?: string;
  fallbackTitle: string;
        ...
        <div className="review-section-actions">
          ...
          {fullWorkspaceDiff.files?.length ?
            <Button
              ...
              variant="outline"
              size="sm"
              disabled={workspaceFileDiffLoading}
              onClick={() => onOpenDiff(fullWorkspaceDiff)}
            >
              Open full diff
            </Button>
          : null}
            ...
            <div className="detail-stack">
              {diff.files.map((file) => (
                <DiffViewer
                  ...
                  diff={file.diff}
                  patch={file.patch}
                  fallbackTitle={file.fallbackTitle}
                />
              ))}
            </div>
      ...
      {files.map((file) => {
        ...
        return (
          <article
            ...
            className={className('changed-file-review-card', expanded && 'active')}
          >
            <button
