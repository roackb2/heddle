...
import { className, formatDate, formatInterval, formatUsage, toneFor } from '../utils';
...
export function HeartbeatWorkspace({
  ...
  const [mobileView, setMobileView] = useState<'list' | 'detail' | 'runs'>(selectedTaskId ? 'detail' : 'list');
  ...
  useEffect(() => {
    ...
    setMobileView((current) => current === 'list' ? 'detail' : current);
  }, [selectedTaskId]);
  ...
  if (showMobileLayout && mobileView === 'list') {
    return (
      <section className="mobile-task-screen mobile-task-list">
  ...
  if (showMobileLayout && mobileView === 'detail') {
    return (
      <section className="mobile-task-screen mobile-task-detail">
        <section className="workspace-main mobile-pane">
          <div className="mobile-task-header">
            <button className="mobile-nav-button" type="button" onClick={() => setMobileView('list')}>← Tasks</button>
          </div>
          <WorkspaceSectionHeader
          ...
          {selectedTask ?
            <div className="mobile-task-meta">
              ...
              <div className="pills compact-pills">
                <button className="mobile-nav-button" type="button" onClick={() => setMobileView('runs')}>Runs</button>
              </div>
            </div>
          : null}
          <div className="task-detail-scroll">
            ...
            : <EmptyState title="No task selected" body="Choose a task from the list to inspect its durable prompt and latest run state." />}
          </div>
        </section>
      </section>
    );
  ...
  if (showMobileLayout && mobileView === 'runs') {
    return (
      <section className="mobile-task-screen mobile-task-runs">
        <aside className="workspace-side mobile-pane">
          <div className="mobile-task-header">
            <button className="mobile-nav-button" type="button" onClick={() => setMobileView('detail')}>← Task</button>
          </div>
          <div className="mobile-task-segmented" role="tablist" aria-label="Task runs view">
            <button className={className(!selectedRunId && 'active')} type="button" onClick={() => setMobileView('detail')}>Summary</button>
            <button className="active" type="button">Runs</button>
          </div>
