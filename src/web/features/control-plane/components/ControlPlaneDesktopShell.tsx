...
import { RuntimeHostStrip, StatusBadge, WorkspacePathLabel, WorkspaceSwitcher } from './common';
import { projectRuntimeHostSurface } from '../host-surface';

export function ControlPlaneDesktopShell({
  state,
  error,
  activeTab,
  sessionPath,
  ...
  error?: string;
  activeTab: 'overview' | 'sessions' | 'heartbeat' | 'workspaces';
  sessionPath: string;
  children: ReactNode;
  onRefresh: () => void;
    ...
    <main className="app-shell">
      <header className="toolbar">
        <nav className="tabs toolbar-tabs" aria-label="Control plane sections">
          <RouteTabLink active={activeTab === 'overview'} to="/overview">Overview</RouteTabLink>
          <RouteTabLink active={activeTab === 'sessions'} to={sessionPath}>Sessions</RouteTabLink>
          <RouteTabLink active={activeTab === 'heartbeat'} to="/tasks">Tasks</RouteTabLink>
          <RouteTabLink active={activeTab === 'workspaces'} to="/workspaces">Workspaces</RouteTabLink>
        </nav>
