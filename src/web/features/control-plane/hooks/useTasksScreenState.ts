...
export type TaskDefinition = ControlPlaneState['heartbeat']['tasks'][number];
export type TaskRun = ControlPlaneState['heartbeat']['runs'][number];

export type TasksScreenState = {
...
export function useTasksScreenState(
  ...
  const [pendingTaskAction, setPendingTaskAction] = useState<TasksScreenState['pendingTaskAction']>();
