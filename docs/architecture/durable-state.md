...
| Settled delegation records | **Conversation-owned persisted metadata** | Reloaded with the completed parent turn | Survives with the owning conversation; active children are not resumed | Follows the configured conversation repository; exact child correlation, agent snapshot, outcome, safe failure, usage, and timestamps survive | Inherits the recent parent-turn summary window and session deletion; no independent child retention or recovery contract |
...
| Raw turn traces | **Workspace-local diagnostic** | Existing root trace files remain | Survives | Does not follow the session unless state storage is shared or copied | No retention or GC policy; session summaries can retain a local-only root `traceFile` locator; raw child traces are not persisted separately |
...
| Agent working set | **Remote-ready portable checkpoint; disposable local copy** | Host-defined | A warm local copy may survive but is discarded before reuse | Restored from the validated committed generation when the host supplies the same authorized scope and durable store | The store owns immutable-generation retention and orphan cleanup; no public delete contract yet |
...
| Agent working set | `WorkingSetCheckpointStore` | Async scoped manifest/generation reads plus immutable-generation-first, manifest compare-and-swap commit | **Remote-ready checkpoint contract**; the host still supplies authorization, a durable adapter, local reset, and lifecycle integration |
...
| Agent working set | Its working-set checkpoint store, addressed by verified adopter, tenant, subject, and stable product-session identity; the Runtime directory is a disposable copy |
| Workspace source files and build state outside the working set | Per-session filesystem for continuity; an explicit domain checkpoint only when cross-version recovery is a selected product promise |
