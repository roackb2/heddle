...
- Browser Automation is off by default. Enabling it activates Heddle's built-in browser guidance and adds browser tools to future default turns. Browser profiles and policy still remain authoritative. Settings -> Browser Automation and `/browser backend <playwright|native-chrome>` choose between the Playwright-managed backend and the experimental native Chrome CDP attach backend. `/browser profile <id>` selects the profile id, `/browser channel <chromium|chrome|msedge>` selects the Playwright channel, and `/browser endpoint <url>` sets the local native Chrome CDP endpoint. `/browser open-profile [url]` only applies to the Playwright-managed backend. Without an explicit domain allowlist, the first opened URL establishes the same-domain browsing boundary. Browser tools can open pages, snapshot rendered state, click current snapshot refs, type into editable snapshot refs for search/navigation, take screenshots, and close the session.
