...
| Settled delegation records | Optional records on the completed parent `TurnSummary` | Included in the opaque conversation record; no separate adapter | Yes, within the recent parent-turn summary window | Yes, through the same scope-bound conversation repository | **Conversation-owned metadata** |
...
| Raw turn traces | Root trace files referenced by a local path; raw child traces stay in memory | No general persistence port | Root files survive on the same storage | No | **Local diagnostic evidence** |
