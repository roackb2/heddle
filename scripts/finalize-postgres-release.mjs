...
function finalizePostgresRelease() {
  assert.equal(
  ...
  const packageJson = JSON.parse(
    readFileSync(
      resolve(repositoryDirectory, 'packages/postgres/package.json'),
      'utf8',
    ),
  );
  const release = createPostgresReleaseMetadata(packageJson);
  const head = run(
    'git',
    ['rev-parse', 'HEAD'],
    repositoryDirectory,
